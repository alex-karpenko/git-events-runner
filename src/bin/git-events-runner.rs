...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd {
        CliCommand::Crds => crdgen(),
        CliCommand::Run {
            webhooks_port,
            utility_port,
            webhooks_parallelism,
            schedule_parallelism,
            secrets_cache_time,
        } => {
            let client = Client::try_default().await?;
            let secrets_cache =
                ExpiringSecretCache::new(Duration::from_secs(secrets_cache_time), client.clone());
            let state = State::new(secrets_cache.clone());
            let identity = Uuid::new_v4().to_string();

            let (mut lock_channel, lock_task) =
                leader_lock::new(&identity, Some("default".into())).await;
            let mut signal_handler = SignalHandler::new().expect("unable to create signal handler");
            let (shutdown_tx, shutdown_rx) = watch::channel(false);
            let mut shutdown = false;

            let utils_web =
                web::build_utils_web(state.clone(), shutdown_rx.clone(), utility_port).await;
            ...
            let (hooks_web, hooks_controller) = {
                let scheduler = SchedulerBuilder::new()
                    ...
                    .parallelism(WorkerParallelism::Limited(webhooks_parallelism as usize))
                    .build();
